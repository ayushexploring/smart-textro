...
public class EfficientDocument extends Document {
	...
	private void processText()
	{
		...
		int wordsCount = 0, sentencesCount = 0, syllablesCount = 0;
		for(String str : tokens) {
			if(isWord(str)) {
				wordsCount++;
				syllablesCount += countSyllables(str);
			}
			else {
				sentencesCount++;
			}
	...
	public static void main(String[] args)
	{
		...
		testCase(new EfficientDocument("Lorem ipsum dolor sit amet, qui ex choro quodsi moderatius, nam dolores explicari forensibus ad."),
		         32, 15, 1);
		EfficientDocument ed = new EfficientDocument("many???  Senteeeeeeeeeences are");
		System.out.println(ed.getTokens("[!?.]+|[a-zA-Z]+"));
	}
